...
const LEAGUE_ALIASES = { 'sau.1': 'ksa.1' };
...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  const goalKey = (minute, player, team, type) =>
    `goal_${normaliseText(minute)}_${normaliseText(player)}_${normaliseText(team)}_${type}`;
  ...
  for (const ev of keyEvents) {
    ...
    const key     = goalKey(fullMin, p1, team, goalType(ev, t));
    ...
    if (t === 'goal' || t === 'owngoal' || t === 'penaltyscored' || (t.includes('goal') && !t.includes('miss')) || (t.includes('penalty') && !t.includes('miss') && !t.includes('save'))) {
      seen.add(key);
      goals.push({ minute: fullMin, player: p1, assist: pp[1]?.athlete?.displayName || '', team, type: goalType(ev, t) });
    }
  ...
  for (const det of details) {
      ...
      const key = goalKey(full, player, det.team?.displayName || '', type);
      if (seen.has(key)) continue;
      seen.add(key);
      goals.push({ minute: full, player, assist: det.participants?.[1]?.athlete?.displayName || '', team: det.team?.displayName || '', type });
  }
  ...
  for (const play of (data.plays || [])) {
    ...
    const key = goalKey(full, player, play.team?.displayName || '', type);
    if (seen.has(key)) continue;
    seen.add(key);
    goals.push({ minute: full, player, assist: pp[1]?.athlete?.displayName || pp[1]?.displayName || '', team: play.team?.displayName || '', type });
  }
...
async function handleScorers(url, env) {
  ...
  if (season && SYNC_SEASONS.includes(season)) {
  ...
  const backupKey = `scorers_backup_${league}_${season || 'def'}`;
  ...
  try {
    ...
    if (!scorers.length) {
      return jsonResp({
        success: false,
        // The browser has a second source and can try the previous season.
...
async function handleScorersSet(url, env, request) {
  ...
  try {
    ...
    const key = `scorers_v4_${league}_${season}`;
    await kvPut(env, key, body, TTL_SCORERS);
    await kvPut(env, `scorers_backup_${league}_${season}`, body, TTL_SCORERS_BACKUP);

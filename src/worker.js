...
const TTL_FINISHED  = null;  // تخزين دائم للمباريات المنتهية
...
async function handleSummary(url, env) {
  ...
  const kvKey  = `summary_v3_${matchId}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  for (const det of details) {
      ...
      const key = `${t || 'goal'}_${full}_${player}`;
      if (seen.has(key)) continue;
      seen.add(key);
      goals.push({ minute: full, player, assist: det.participants?.[1]?.athlete?.displayName || '', team: det.team?.displayName || '', type: det.ownGoal || t === 'owngoal' ? 'ownGoal' : det.penaltyKick || t.includes('penalty') ? 'penalty' : 'goal' });
  }
  ...
  for (const play of (data.plays || [])) {
    ...
    if (seen.has(key)) continue;
    seen.add(key);
    goals.push({ minute: full, player, assist: pp[1]?.athlete?.displayName || pp[1]?.displayName || '', team: play.team?.displayName || '', type: play.ownGoal || t === 'owngoal' ? 'ownGoal' : play.penaltyKick || t.includes('penalty') ? 'penalty' : 'goal' });
  }

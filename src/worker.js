...
async function handleStandings(url, env) {
  ...
  const standingsVersion = league === 'caf.nations' ? 'standings_v8' : null;
  const kvKey = requestedSeason
    ? `${standingsVersion || 'standings_v7'}_${league}_${requestedSeason}`
    : `${standingsVersion || 'standings_v5'}_${league}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  ...
  await Promise.all(dayKeys.map(async (dateParam) => {
    ...
    try {
      ...
      for (const m of (data?.matches || [])) {
        ...
        urls.add(`${origin}/index.html?${qs.toString()}`);
      }
    } catch {

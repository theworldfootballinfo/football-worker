...
function extractScorers(data) {
  // ① شكل statistics: data.stats[].leaders
  const statsList = data.stats || data.statistics || [];
  if (statsList.length) {
    const goalCat = statsList.find(s =>
      (s.name||'').toLowerCase().includes('goal') ||
      (s.abbreviation||'').toUpperCase() === 'G' ||
      (s.displayName||'').toLowerCase().includes('goal')
    ) || statsList[0];
    if ((goalCat?.leaders || []).length) {
      return (goalCat.leaders).map((l, i) => ({
        rank: i + 1,
  ...
  // ② شكل leaders: data.leaders[{name:'goalLeaders',leaders:[]}]
  const leadersList = data.leaders || [];
  if (leadersList.length) {
    const goalCat = leadersList.find(c =>
      (c.name||'').toLowerCase().includes('goal') ||
      (c.displayName||'').toLowerCase().includes('goal')
    ) || leadersList[0];
    const inner = goalCat?.leaders || goalCat?.athletes || [];
    if (inner.length) {
...
async function handleScorers(url, env) {
  ...
  try {
    ...
    const tryUrls = [
      ...
      // leaders endpoint — أكثر موثوقية لبعض الدوريات

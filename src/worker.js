// ═══════════════════════════════════════════════════════════════════════════════
// worker.js — الحل الصحيح بناءً على اختبارات User-Agent
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══ المفتاح: استخدام User-Agent يعرف التطبيق وليس متصفحاً مزيفاً ═══
function getHeaders() {
  return {
    'User-Agent': 'Scorio/1.0 (Cloudflare Worker; +https://scorio.com)',
...
async function safeFetch(url) {
  ...
  try {
    ...
    if (!contentType.includes('application/json')) {
      ...
      console.error(`❌ ليس JSON: ${text.substring(0, 200)}`);
      return { success: false, error: 'Not JSON response', html: text.substring(0, 200) };
    }
...
async function handleMatches(url, env) {
  ...
  try {
    ...
    const result = await safeFetch(espnUrl);
    ...
    if (!result.success || !result.data) {
      return jsonResp({
        ...
        count: 0,
        matches: [],
        error: result.error || 'فشل جلب البيانات',
    ...
    const response = { success: true, date, count: matches.length, matches };
    await kvPut(env, kvKey, response, hasLive ? 120 : isToday ? 300 : null);
...
async function handleSummary(url, env) {
  ...
  const kvKey = `summary_v10_${matchId}`;
  const cached = await kvGet(env, kvKey);
  if (cached) {
    return jsonResp({ ...cached, fromCache: true });
  }
  ...
  for (const lg of leaguesToTry) {
    try {
      const espnUrl = `https://site.api.espn.com/apis/site/v2/sports/soccer/${lg}/summary?event=${matchId}`;
      console.log(`📡 محاولة summary مع: ${lg}`);
      const result = await safeFetch(espnUrl);
      if (result.success && result.data?.header?.competitions?.[0]?.competitors?.length) {
        data = result.data;
        usedLeague = lg;
        break;
      }
...
async function handleStandings(url, env) {
  ...
  const kvKey = `standings_v10_${league}`;
  ...
  try {
    const espnUrl = `https://site.api.espn.com/apis/v2/sports/soccer/${league}/standings`;
    console.log(`📡 جلب ترتيب: ${league}`);
    const result = await safeFetch(espnUrl);
    
    if (!result.success || !result.data) {
      return jsonResp({ success: false, groups: [], error: result.error || 'فشل جلب الترتيب' }, 502);
    }
    ...
    const resultPayload = { success: true, league, groups };
    await kvPut(env, kvKey, resultPayload, 21600);
    return jsonResp(resultPayload);
...
async function handleScorers(url, env) {
  ...
  const kvKey = `scorers_v10_${league}_${season}`;
  ...
  try {
    const espnUrl = `https://site.api.espn.com/apis/v2/sports/soccer/${league}/statistics?season=${season}`;
    console.log(`📡 جلب هدافين: ${league} موسم ${season}`);
    const result = await safeFetch(espnUrl);
    
    if (!result.success || !result.data) {
      return jsonResp({ success: false, scorers: [], error: result.error || 'فشل جلب الهدافين' }, 502);
    }
    ...
    const resultPayload = { success: true, league, season, scorers };
    await kvPut(env, kvKey, resultPayload, 21600);
    return jsonResp(resultPayload);

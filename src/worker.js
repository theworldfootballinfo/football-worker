...
async function handleScorers(url, env) {
  ...
  try {
    ...
    const tryUrls = [
      // statistics endpoint — كلا الموسمَين
      ...
      `https://site.api.espn.com/apis/site/v2/sports/soccer/${league}/leaders?season=${prevSeason}`,
      `https://site.api.espn.com/apis/site/v2/sports/soccer/${league}/leaders`,
      // ESPN core API — الأكثر موثوقية لكثير من الدوريات
      `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${league}/seasons/${season}/types/2/leaders?limit=50`,
      `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${league}/seasons/${prevSeason}/types/2/leaders?limit=50`,
      // بدون موسم
...
async function handleLeagueRounds(url, env) {
  ...
  if (cached) return jsonResp({ ...cached, fromCache: true });
  try {
    // جرّب عدة صيغ لجلب التقويم
    let data = null;
    for (const u of [
      `${ESPN_BASE}/${league}/scoreboard?season=${season}&limit=200`,
      `${ESPN_BASE}/${league}/scoreboard?season=${parseInt(season)-1}&limit=200`,
      `${ESPN_BASE}/${league}/scoreboard`,
    ]) {
      try {
        const r = await fetch(u, { headers: { 'User-Agent': 'Scorio/1.0' } });
        if (!r.ok) continue;
        const d = await r.json();
        const cal = d.leagues?.[0]?.calendar || [];
        if (cal.length) { data = d; break; }
      } catch { continue; }
    }
    if (!data) return jsonResp({ success: true, league, type: 'matchdays', rounds: [] });

...
async function handleSummary(url, env) {
  ...
  const leaguesToTry = league
    ? [league, 'eng.1','esp.1','ger.1','ita.1','fra.1','bra.1','arg.1','ned.1','por.1','ksa.1','usa.1','conmebol.libertadores','uefa.champions','fifa.world']
    : ['eng.1','esp.1','ger.1','ita.1','fra.1','bra.1','arg.1','ned.1','por.1','ksa.1','usa.1','conmebol.libertadores','uefa.champions','fifa.world'];
...
// Leagues whose ESPN code differs from the UI code
const STANDINGS_CODE_FIXES = {
  'sau.1': 'ksa.1',  // Saudi Pro League: sau.1 → 400, ksa.1 → 200
};
...
async function handleStandings(url, env) {
  ...
  // Remap codes that ESPN doesn't recognise under the common alias
  const espnLeague = STANDINGS_CODE_FIXES[league] || league;
  const kvKey  = `standings_v5_${league}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });

  const seasonParam = getSeasonParam(espnLeague);
  // Try the corrected ESPN code first, then without a season param as fallback
  const urls = [
    `${ESPN_STAND}/${espnLeague}/standings${seasonParam}`,
    `${ESPN_STAND}/${espnLeague}/standings`,
  ];
  let data = null;
  for (const u of urls) {
    try {
...
// Calculate current football season year (e.g. Aug 2025–Jul 2026 = season 2025)
...
async function handleScorers(url, env) {
  ...
  if (!isNaN(league) && ID_TO_CODE[league]) league = ID_TO_CODE[league];
  const espnLeague = STANDINGS_CODE_FIXES[league] || league;
  const kvKey  = `scorers_v4_${league}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });

  // ESPN core API: /leagues/{league}/seasons/{year}/types/1/leaders
  const season = SEASON_OVERRIDE[espnLeague] || currentSeasonYear();
  const leagueName = LEAGUE_NAMES[league] || LEAGUE_NAMES[espnLeague] || league;

  // Try current season, then fallback to previous season
  ...
  for (const yr of seasons) {
    try {
      const res = await fetch(
        `${ESPN_CORE}/${espnLeague}/seasons/${yr}/types/1/leaders?limit=20`,
        { headers: { 'User-Agent': 'Scorio/1.0' } }
      );
      if (!res.ok) continue;
      const data = await res.json();
      const cats = data.categories || [];
      const goalsCat = cats.find(c => c.name === 'goalsLeaders' || (c.name||'').toLowerCase().includes('goal')) || cats[0];
      if (goalsCat?.leaders?.length) { leaders = goalsCat.leaders; break; }
    } catch { continue; }
  }

  if (!leaders.length) {
    return jsonResp({ success: true, league, leagueName, scorers: [], noData: true });
  }

  // Fetch athlete refs and team refs in parallel (top 20 leaders)
  const top = leaders.slice(0, 20);
  const [athletes, teams] = await Promise.all([
    Promise.all(top.map(l => l.athlete?.$ref
      ? fetch(l.athlete.$ref, { headers: { 'User-Agent': 'Scorio/1.0' } }).then(r => r.ok ? r.json() : null).catch(() => null)
      : Promise.resolve(null)
    )),
    Promise.all(top.map(l => l.team?.$ref
      ? fetch(l.team.$ref, { headers: { 'User-Agent': 'Scorio/1.0' } }).then(r => r.ok ? r.json() : null).catch(() => null)
      : Promise.resolve(null)
    )),
  ]);

  const scorers = top.map((l, i) => ({
    rank: i + 1,
    name: athletes[i]?.displayName || '',
    photo: athletes[i]?.headshot?.href || '',
    team: teams[i]?.displayName || teams[i]?.name || '',
    teamLogo: teams[i]?.logos?.[0]?.href || '',
    goals: parseInt(l.value) || 0,
  })).filter(s => s.name && s.goals > 0);

  const result = { success: true, league, leagueName, scorers, noData: scorers.length === 0 };
  if (scorers.length) await kvPut(env, kvKey, result, TTL_SCORERS);
  return jsonResp(result);
}

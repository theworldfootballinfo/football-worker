  ...
  for (const lg of leaguesToTry) {
    try {
      ...
      if (d.header?.competitions?.[0]?.competitors?.length) {
        ...
        const _nameForResolve = d.header?.league?.name || d.header?.league?.shortName || '';
        if (espnId && ID_TO_CODE[espnId]) {
          usedLeague = ID_TO_CODE[espnId];
        } else if (_nameForResolve && resolveLeagueFromName(_nameForResolve)) {
          usedLeague = resolveLeagueFromName(_nameForResolve);
        } else if (espnId) {
          usedLeague = espnId;
        } else if (league && league.includes('.')) {
          usedLeague = league;
        }
        break;
      }
...
async function handleScorers(url, env) {
  let league = url.searchParams.get('league') || 'eng.1';
  if (!isNaN(league) && ID_TO_CODE[league]) league = ID_TO_CODE[league];
  const kvKey  = `scorers_v2_${league}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  try {
    const seasonParam = getSeasonParam(league);
    const res  = await fetch(`${ESPN_BASE}/${league}/leaders${seasonParam}`, { headers: { 'User-Agent': 'Scorio/1.0' } });
    if (!res.ok) throw new Error(`ESPN ${res.status}`);
    const data = await res.json();
    const cats = data.categories || [];
    const goalsCat = cats.find(c => (c.name||'').toLowerCase().includes('goal')) || cats[0];

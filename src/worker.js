...
async function handleSitemap(url, env) {
  ...
  for (const league of Object.keys(LEAGUE_NAMES)) {
    const q = new URLSearchParams({ league, lang: 'ar' });
  ...
  // Read the match caches already maintained by this Worker.
  // We intentionally do not fetch ESPN here: opening a sitemap should not
  // create a new upstream workload or alter the existing match cache logic.
  const now = new Date();
  const days = [];
  ...
  await Promise.all(days.map(async (date) => {
    try {
      ...
      for (const m of cached.matches) {
        ...
        urls.add(`${origin}/index.html?${q.toString()}`);
      }
    } catch {}
  }));
  ...
  const body = [
    ...
    ...Array.from(urls).sort().map(u => `  <url><loc>${xmlEscape(u)}</loc></url>`),
    '</urlset>'
  ].join('\n');

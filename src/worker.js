...
async function handleStandings(url, env) {
  ...
  if (!data) {
    const isCup = ['cup','copa','coupe','pokal','fa','league_cup','qual',
      'libertadores','sudamericana','worldq','confed','nations_qual']
      .some(k => league.includes(k));
    return jsonResp({ success: false, noStandings: true, isCup, groups: [],
      message: isCup ? 'هذه البطولة لا تحتوي على ترتيب تقليدي' : 'لا توجد بيانات ترتيب' });
  }

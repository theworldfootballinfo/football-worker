...
const ID_TO_CODE = {
  ...
  '2350':'afc.champions.elite','3901':'bel.1','3907':'aut.1','3914':'eng.2','3921':'esp.2',
  '3904':'arg.2','3930':'irl.1','3945':'swe.1',
...
function resolveLeagueFromName(name) {
  ...
  if (n.includes('belgian') || n.includes('jupiler') || n.includes('belgië') || n.includes('belgique')) return 'bel.1';
  if (n.includes('championship') || n.includes('efl championship') || n.includes('english league championship')) return 'eng.2';
  if (n.includes('laliga 2') || n.includes('la liga 2') || n.includes('segunda división') || n.includes('segunda division') || n.includes('segunda española')) return 'esp.2';
  // ESPN calls both German and Austrian competitions "Bundesliga".
  // Resolve Austria before the generic Bundesliga rule.
  if (n.includes('austrian') || n.includes('österreich') || n.includes('autrich') || n.includes('austria')) return 'aut.1';
...
function parseEvent(ev, expectedLeague = '') {
  ...
  const detectedLeague = canonicalLeagueCode(ID_TO_CODE[leagueId]
    || resolveLeagueFromName(ev.leagues?.[0]?.name || comp.altGameNote?.split(',')?.[0] || '')
    || leagueId || '');
  // A league-scoped endpoint is authoritative even when ESPN introduces a
  // new numeric competition id that is not in the map yet.
  const leagueCode = canonicalLeagueCode(expectedLeague || detectedLeague);

...
const ID_TO_CODE = {
  ...
  '5672':'afc.asean','8301':'usa.nwsl','8376':'chn.1',
  '11088':'bra.3','23286':'can.1','221':'can.1','19834':'friendly',
  '186':'uae.1','190':'qat.1','1125':'qat.1','194':'bhr.1','332':'bhr.1',
  '198':'omn.1','335':'omn.1','206':'jor.1','333':'jor.1',
  '210':'irq.1','343':'irq.1','214':'lbn.1','336':'lbn.1',
  '218':'kwt.1','334':'kwt.1','202':'syr.1','338':'syr.1',
  '341':'irn.1','234':'egy.1','1123':'egy.1','231':'mar.1','1121':'mar.1',
  '233':'alg.1','1118':'alg.1','232':'tun.1','1133':'tun.1',
  '236':'nga.1','235':'rsa.1','238':'gha.1','108':'rou.1',
  '113':'irl.1','129':'svk.1','128':'svn.1','166':'ind.1',
  '167':'kor.1','178':'tha.1','179':'mas.1','180':'idn.1','182':'vie.1',
};
...
async function resolveLeagueSlug(leagueId, env) {
  ...
  if (s.includes('.')) return s; // slug صحيح مثل rus.1
  ...
  // 3. جرب resolveLeagueFromName إذا كان leagueName متاحاً
  // (سيتم استدعاؤه من normalizeLeague قبل هذه الدالة)

  // 4. اسأل ESPN عبر all/scoreboard — يحتوي slug لكل دوري
  try {
    ...
    if (res.ok) {
      ...
      for (const ev of (data.events || [])) {
        ...
        if (m && m[1] === s) {
          // وجدنا المباراة — نجلب slug من leagueId
          const slug = ev.leagues?.[0]?.slug || '';
          if (slug && slug.includes('.')) {
            try { await env?.FOOTBALL_KV?.put(kvKey, slug); } catch {}
            return slug;
          }
          // نجرب scoreboard بالـ slug من altGameNote
          const altNote = ev.competitions?.[0]?.altGameNote || '';
          const nameSlug = altNote.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');
          if (nameSlug) {
            try {
              const r2 = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/${nameSlug}/scoreboard`,
                { headers: { 'User-Agent': 'Scorio/1.0' } });
              if (r2.ok) {
                const d2 = await r2.json();
                const s2 = d2.leagues?.[0]?.slug || '';
                if (s2 && s2.includes('.')) {
                  try { await env?.FOOTBALL_KV?.put(kvKey, s2); } catch {}
                  return s2;
                }
              }
            } catch {}
...
function resolveLeagueFromName(name) {
  ...
  if (n.includes('liga auf uruguaya')) return 'uru.1';
  if (n.includes('liga auf')) return 'uru.1';
  ...
  if (n.includes('asean champ')) return 'afc.asean';
  if (n.includes('nwsl')) return 'usa.nwsl';
  if (n.includes('mls') || n.includes('major league soccer')) return 'usa.1';
  if (n.includes('saudi') || n.includes('roshn')) return 'sau.1';
  if (n.includes('j1 league') || n.includes('j-league')) return 'jpn.1';

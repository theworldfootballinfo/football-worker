...
const ID_TO_CODE = {
  ...
  '8301':'usa.nwsl','11088':'bra.3','23286':'can.1','221':'can.1',
...
async function handleLeagueMatches(url, env) {
  ...
  league = normalizeLeague(league, leagueName);
  const kvKey  = `lgm_v3_${league}_${dateFrom}_${dateTo}`;
  const isToday = dateFrom === todayEspn() || dateTo === todayEspn();
  const cached  = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  try {
    const dates = [];
    let cur = new Date(dateFrom.slice(0,4)+'-'+dateFrom.slice(4,6)+'-'+dateFrom.slice(6,8));
    const end = new Date(dateTo.slice(0,4)+'-'+dateTo.slice(4,6)+'-'+dateTo.slice(6,8));
    while (cur <= end && dates.length < 8) {
      dates.push(cur.toISOString().slice(0,10).replace(/-/g,''));
      cur.setDate(cur.getDate() + 1);
    }
    const fetchDay = async (d) => {
      try {
        const res = await fetch(`${ESPN_BASE}/${league}/scoreboard?dates=${d}&limit=100`, { headers: { 'User-Agent': 'Scorio/1.0' } });
        if (!res.ok) return [];
        const data = await res.json();
        return (data.events || []).map(ev => parseEvent(ev));
      } catch { return []; }
    };
    const allMatches = (await Promise.all(dates.map(fetchDay))).flat();
    const seen = new Set();
    const matches = allMatches.filter(m => { if (seen.has(m.id)) return false; seen.add(m.id); return true; });
    matches.sort((a,b) => new Date(a.date) - new Date(b.date));
    const hasLive = matches.some(m => m.status === 'in');
    const result = { success: true, league, dateFrom, dateTo, leagueName: LEAGUE_NAMES[league] || league, count: matches.length, matches };
    await kvPut(env, kvKey, result, hasLive ? TTL_LIVE : isToday ? TTL_MATCHES : TTL_FINISHED);
    return jsonResp(result);
  } catch (e) {
    return jsonResp({ success: false, error: e.message }, 500);

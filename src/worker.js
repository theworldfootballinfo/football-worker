...
const ID_TO_CODE = {
  ...
  '725':'ned.1','715':'por.1','3901':'bel.1','3907':'aut.1',
...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  const eventMinute = event => {
    ...
    const added = event.addedClock?.displayValue;
    return minute ? `${minute}${added ? `+${added}` : ''}` : '';
  };
  const goalType = (event, type) =>
    event.ownGoal || type === 'owngoal' ? 'ownGoal' :
    event.penaltyKick || type.includes('penalty') ? 'penalty' : 'goal';
  const addGoal = goal => {
    // ESPN's `event.team` is the team credited with the goal, including
    // own-goal events. Keep it unchanged so the event stays under the team
    // whose score was increased.
  ...
  for (const play of (data.plays || [])) {
    ...
    const playerOut = pOut.athlete?.displayName || pOut.athlete?.shortName || pOut.displayName || '';
    const playerIn  = pIn.athlete?.displayName  || pIn.athlete?.shortName  || pIn.displayName || '';
    if (!playerOut && !playerIn) continue;
    addSub({
      minute: full,
      playerIn: playerIn || '—',
      playerOut: playerOut || '—',
      jerseyIn: jerseyMap[playerIn] || '',
      jerseyOut: jerseyMap[playerOut] || '',
      team,
    });
  }

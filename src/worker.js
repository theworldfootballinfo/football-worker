// ═══════════════════════════════════════════════════════════════════════════════
// worker.js — الحل النهائي: استخدام User-Agent الصحيح
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══ المفتاح: User-Agent الذي يعمل مع ESPN ═══
const HEADERS = {
...
// ─── جلب مباشر مع User-Agent الصحيح ──────────────────────────────────────────
...
async function handleMatches(url, env) {
  ...
  const result = await fetchESPN(espnUrl);
...
async function handleSummary(url, env) {
  ...
  const kvKey = `summary_v11_${matchId}`;
  const cached = await kvGet(env, kvKey);
  if (cached) {
    return jsonResp({ ...cached, fromCache: true });
  }
...
async function handleStandings(url, env) {
  ...
  const kvKey = `standings_v11_${league}`;
...
async function handleScorers(url, env) {
  ...
  const kvKey = `scorers_v11_${league}_${season}`;

// ═══════════════════════════════════════════════════════════════
// worker.js — Scorvex Cloudflare Worker (نسخة كاملة مع مزامنة تاريخية)
// ═══════════════════════════════════════════════════════════════
...
// CONMEBOL's standings endpoint currently sends qualification notes only for
// the first group. The same stage rules apply to every group, so create the
// fallback per group instead of reusing a colour map produced by Group A.
function conmebolGroupColor(league, position) {
  const rules = league === 'conmebol.libertadores'
    ? {
        1: { color: '#81D6AC', desc: 'Qualifies for Round of 16' },
        2: { color: '#B5E7CE', desc: 'Qualifies for Round of 16' },
        3: { color: '#B2BFD0', desc: 'Qualifies for Sudamericana KO Playoffs' },
      }
    : league === 'conmebol.sudamericana'
      ? {
          1: { color: '#81D6AC', desc: 'Qualifies for Round of 16' },
          2: { color: '#B5E7CE', desc: 'Qualifies for KO Playoffs' },
        }
      : {};
  return rules[Number(position)] || { color: '', desc: '' };
}
...
const ID_TO_CODE = {
  ...
  '2350':'afc.champions','3904':'arg.2','3930':'irl.1','3945':'swe.1',
  ...
  '20296':'uefa.europa.conf','21604':'afc.asian.cup','23286':'can.1',
  '3956':'ita.coppa_italia',
  // Current ESPN league ids. ESPN's event uid uses these ids even when the
  // public league slug is different. Keep both legacy and current ids so
  // all-match feeds can be normalised without guessing from a URL.
  '700':'eng.1','3914':'eng.2','3915':'eng.3','3916':'eng.4','3917':'eng.5',
  '740':'esp.1','3921':'esp.2','730':'ita.1','3931':'ita.2',
  '720':'ger.1','3927':'ger.2','710':'fra.1','3926':'fra.2',
  '725':'ned.1','715':'por.1','3901':'bel.1','3907':'aut.1','3908':'caf.nations',
  '3944':'sui.1','3955':'gre.1','3946':'tur.1','735':'sco.1',
  '3913':'den.1','3960':'nor.1','3945':'swe.1','3939':'rus.1',
  '630':'bra.1','745':'arg.1','760':'mex.1','770':'usa.1',
  '750':'jpn.1','8376':'chn.1','3906':'aus.1','21231':'ksa.1',
  '775':'uefa.champions','776':'uefa.europa','777':'uefa.europa.conf',
  '2466':'afc.cup','3902':'afc.champions',
};
...
const LEAGUE_ALIASES = {
  ...
  'afc.champions.elite': 'afc.champions',
  'afc.champions.2': 'afc.cup',
};
...
function resolveLeagueFromName(name) {
  ...
  // Check lower divisions and country-specific Bundesliga names before the
  // broad top-division rules below. Without this order, "LALIGA 2",
  // "2. Bundesliga" and "Austrian Bundesliga" are all misclassified.
  if (n.includes('laliga 2') || n.includes('la liga 2') ||
      n.includes('segunda división') || n.includes('segunda division') ||
      n.includes('liga hypermotion')) return 'esp.2';
  if (n.includes('efl championship') || n.includes('english league championship') ||
      n.includes('championship')) return 'eng.2';
  if (n.includes('efl league one') || n.includes('english league one')) return 'eng.3';
  if (n.includes('efl league two') || n.includes('english league two')) return 'eng.4';
  if (n.includes('2. bundesliga') || n.includes('2 bundesliga')) return 'ger.2';
  if (n.includes('austrian bundesliga') || n.includes('österreichische bundesliga') ||
      n.includes('bundesliga autrichienne')) return 'aut.1';
  if (n.includes('belgian pro league') || n.includes('jupiler pro league')) return 'bel.1';
  if (n.includes('coppa italia') || n.includes('italian cup')) return 'ita.coppa_italia';
  ...
  // AFC/CAF names must be checked before the broad "Champions League"
  // rule, otherwise both are incorrectly classified as UEFA Champions League.
  if (n.includes('afc champions league two') ||
      n.includes('afc champions league 2') ||
      n.includes('afc champions two')) return 'afc.cup';
  ...
  if (n.includes('primeira liga') || n.includes('liga portugal') || n.includes('portugal')) return 'por.1';
...
const SEASON_OVERRIDE = {
  ...
  // AFCON 2025 is played across Dec 2025 / Jan 2026. ESPN exposes its
  // standings under the 2025 competition season.
  'caf.nations':'2025',
};
...
const LEAGUE_NAMES = {
  ...
  'caf.nations':'كأس الأمم الأفريقية','caf.confed':'كأس الكونفدرالية الأفريقية',
  'afc.champions':'دوري أبطال آسيا للنخبة','afc.cup':'دوري أبطال آسيا 2',
...
function parseEvent(ev, fallbackLeague = '') {
  const comp    = ev.competitions?.[0] || {};
  const home    = comp.competitors?.find(c => c.homeAway === 'home') || {};
  const away    = comp.competitors?.find(c => c.homeAway === 'away') || {};
  const status  = ev.status?.type || {};
  const uid     = ev.uid || '';
  // The league segment in the event UID is the authoritative competition
  // identifier. Do not infer a competition from display fields: ESPN can
  // expose a stage label there, and old responses may contain unrelated
  // numeric values in `league`.
  const leagueId = uid.match(/l:(\d+)/)?.[1] || '';
  const rawLeagueName = comp.altGameNote?.split(',')?.[0] ||
    ev.leagues?.[0]?.name || '';
  // The numeric league id in the event uid is the authoritative source for
  // the all-competitions feed. altGameNote is display text and can be a
  // generic competition/stage label, so resolving it first can place a
  // match under the wrong league.
  const leagueCode = canonicalLeagueCode(
    ID_TO_CODE[leagueId] ||
    fallbackLeague ||
    resolveLeagueFromName(rawLeagueName) ||
    leagueId || ''
  );
  ...
  const leagueNameOnly = parts[0] || ev.leagues?.[0]?.name || leagueCode;
  // ESPN's event uid uses the league segment as the stable tournament
  // identifier: s:600~l:8376~e:401861545. Keep the raw id in the response
  // so the client can group each competition independently of its display
  // name or ESPN code.
  const tournamentId = leagueId || '';
  ...
  return {
    id: ev.id, leagueId, tournamentId, league: leagueCode, leagueName, leagueFlag,
    leagueNameOnly,
    leagueNameRaw: leagueNameOnly,
...
async function handleMatches(url, env) {
  ...
  // v6 invalidates results written before the UID league id was authoritative.
  const kvKey   = `matches_v6_${date}`;
  const isToday = date === todayEspn();
  const cached  = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  try {
    const res     = await fetch(`${ESPN_ALL}?dates=${date}&limit=500`, { headers: { 'User-Agent': 'Scorvex/1.0' } });
...
async function handleSummary(url, env) {
  ...
  // إصدار جديد حتى لا تُعاد نتائج قديمة ناقصة الأحداث أو بإسناد خاطئ
  // للأهداف العكسية من الكاش.
  // Bump this whenever the event payload shape changes; otherwise an older
  // cached summary can keep returning substitutions without jersey numbers.
  ...
  const leaguesToTry = [...new Set([
    ...
    'uefa.europa','uefa.europa.conf','afc.champions','afc.cup',
  ...
  for (let offset = 0; offset < leaguesToTry.length && !data; offset += 8) {
    ...
    const results = await Promise.all(batch.map(async lg => {
      try {
        const res = await fetch(`${ESPN_BASE}/${lg}/summary?event=${matchId}`, { headers: { 'User-Agent': 'Scorvex/1.0' } });
    ...
    if (found) {
      ...
      const headerLeagueName = data.header?.league?.name ||
        data.header?.competitions?.[0]?.altGameNote?.split(',')?.[0] || '';
      // Resolve the competition name first. This prevents LALIGA 2 and
      // Austrian Bundesliga from being collapsed into their top divisions.
      usedLeague   = resolveLeagueFromName(headerLeagueName) ||
        ((espnId && ID_TO_CODE[espnId]) ? ID_TO_CODE[espnId] : '') ||
        league || found.lg;
    }
  }
...
function extractEvents(data, homeTeamName, awayTeamName) {
  ...
  const oppositeTeam = team => {
    ...
    if (teamKey && teamKey === normaliseText(homeTeamName)) return awayTeamName;
    if (teamKey && teamKey === normaliseText(awayTeamName)) return homeTeamName;
    return team;
  };
  const addGoal = goal => {
    // ESPN's own-goal event normally carries the offender's team. The goal
    // must be grouped with the opponent because that is the team credited
    // with the score.
    if (goal.type === 'ownGoal') {
      goal = { ...goal, team: oppositeTeam(goal.team) };
    }
  ...
  // Roster names and event participant names are not always identical:
  // one endpoint may return "João Pedro", another "Joao Pedro", a short name,
  // or only an athlete id. Build several lookup keys so substitutions can
  // reuse the jersey number already shown in the lineup.
  const jerseyMap = {};
  const jerseyLastNameMap = {};
  const jerseyKey = value => String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9\u0600-\u06ff]+/g, ' ')
    .trim()
    .replace(/\s+/g, ' ');
  const addJersey = (value, jersey) => {
    const key = jerseyKey(value);
    if (key && jersey !== undefined && jersey !== null && String(jersey) !== '') {
      jerseyMap[key] = String(jersey);
      const last = key.split(' ').pop();
      if (last && !jerseyLastNameMap[last]) jerseyLastNameMap[last] = String(jersey);
    }
  };
  for (const ro of (data.rosters || [])) {
    for (const p of (ro.roster || [])) {
      ...
      [
        ...
        athlete.name, p.displayName, p.name, athlete.id, athlete.uid
      ].forEach(value => addJersey(value, jersey));
    }
  }
  const jerseyFor = (participant, fallbackName) => {
    const athlete = participant?.athlete || participant || {};
    const direct = participant?.jersey ?? athlete.jersey;
    if (direct !== undefined && direct !== null && String(direct) !== '') return String(direct);
    const candidates = [
      athlete.displayName, athlete.fullName, athlete.shortName, athlete.name,
      participant?.displayName, fallbackName, athlete.id, athlete.uid
    ].map(jerseyKey).filter(Boolean);
    for (const key of candidates) {
      if (jerseyMap[key]) return jerseyMap[key];
    }
    for (const key of candidates) {
      const last = key.split(' ').pop();
      if (last && jerseyLastNameMap[last]) return jerseyLastNameMap[last];
    }
    return '';
  };
  ...
  for (const play of (data.plays || [])) {
    ...
    if (!playerOut && !playerIn) continue;
     addSub({
       ...
       playerIn: playerIn || '—',
       playerOut: playerOut || '—',
       jerseyIn: jerseyFor(pIn, playerIn),
       jerseyOut: jerseyFor(pOut, playerOut),
       team
     });
  }
  ...
  if (subs.length === 0) {
    for (const ev of keyEvents) {
      ...
      const isOut = t === 'substitutionout';
       addSub({
         minute: full,
         playerIn: (isOut ? p2 : p1) || '—',
         playerOut: (isOut ? p1 : p2) || '—',
         jerseyIn: jerseyFor(isOut ? pp[1] : pp[0], isOut ? p2 : p1),
         jerseyOut: jerseyFor(isOut ? pp[0] : pp[1], isOut ? p1 : p2),
         team
       });
    }
  }
...
async function handleStandings(url, env) {
  ...
  // AFCON used to be cached with the rolling current-season key. Use a new
  // key for it so the correct 2025 group tables replace any stale response.
  // Bump the namespace after correcting league ids/slugs so stale tables
  // created with the old mapping cannot be returned.
  const standingsVersion = 'standings_v10';
  const kvKey = requestedSeason
    ? `${standingsVersion}_${league}_${requestedSeason}`
    : `${standingsVersion}_${league}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  ...
  for (const u of urls) {
    try {
      const res = await fetch(u, { headers: { 'User-Agent': 'Scorvex/1.0' } });
      if (!res.ok) continue;
      const d = await res.json();
      const children = Array.isArray(d.children) && d.children.length
        ? d.children
        : (d.standings?.entries || d.entries || []).length
          ? [{ name: '', standings: d.standings?.entries ? d.standings : { entries: d.entries } }]
          : [];
      if (children.some(c => (c.standings?.entries || c.entries || []).length > 0)) {
        data = { ...d, children };
        break;
      }
    } catch { continue; }
  ...
  const groups = [];
  const standingGroups = data.children || [];
  for (const child of standingGroups) {
    const groupName = standingGroups.length > 1 ? (child.name || '') : '';
    ...
    const rangeMap  = buildRangeColorMap(
      ...
      (standingGroups.length === 1 ? data.notes : [])
    );
    const teams = entries.map((e, i) => {
      const team  = e.team || {};
      const stats = {};
      for (const s of (e.stats || [])) stats[s.name] = s.displayValue ?? s.value ?? 0;
      const rank = parseInt(stats.rank) || 0;
      let zone = resolveStandingColor(rank, e.note, rangeMap);
      if (!zone.color && (league === 'conmebol.libertadores' || league === 'conmebol.sudamericana')) {
        zone = conmebolGroupColor(league, rank);
      }
      return {
    ...
    teams.sort((a, b) => {
      // ESPN normally supplies rank. If it is missing (as it can be for
      // AFCON group tables), use the official table tie-break sequence.
      if (a.rank > 0 && b.rank > 0 && a.rank !== b.rank) return a.rank - b.rank;
      if (a.points !== b.points) return b.points - a.points;
      if (a.gd !== b.gd) return Number(b.gd || 0) - Number(a.gd || 0);
      if (a.wins !== b.wins) return b.wins - a.wins;
      return a.name.localeCompare(b.name);
    });
    teams.forEach((team, index) => {
      if (!team.rank) team.rank = index + 1;
    });
...
async function handleScorers(url, env) {
  ...
  try {
    const requestedSeason = season ||
      getSeasonParam(league).match(/season=(\d{4})/)?.[1] || currentSeason;
    const seasonCandidates = [requestedSeason];
    if (requestedSeason === currentSeason) {
      seasonCandidates.push(String(Number(requestedSeason) - 1));
    }
    // When a season is selected, never query an unscoped endpoint: ESPN's
    // unscoped response may be the previous completed season.
    const urlsToTry = seasonCandidates.flatMap(candidateSeason => [
      `${ESPN_BASE}/${league}/statistics?season=${candidateSeason}`,
      `${ESPN_BASE}/${league}/leaders?season=${candidateSeason}`,
    ]);

    let scorers = [];
    let sourceSeason = requestedSeason;

    for (const u of urlsToTry) {
      try {
        const queriedSeason = u.match(/[?&]season=(\d{4})/)?.[1] || requestedSeason;
        const res = await fetch(u, { headers: { 'User-Agent': 'Scorvex/1.0' } });
        if (!res.ok) continue;
        const data = await res.json();
    ...
    const result = {
      ...
      league,
      season: requestedSeason,
      ...(sourceSeason !== requestedSeason ? { sourceSeason, stale: true } : {}),
      leagueName: LEAGUE_NAMES[league] || league,
      scorers,
    };
...
async function handleLeagueMatches(url, env) {
  ...
  // v5 invalidates league-match caches created with the old numeric fallback.
  const kvKey = `lgm_v5_${league}_${season || 'current'}_${dateFrom}_${dateTo}`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
    ...
    await Promise.all(dates.map(async (d) => {
      try {
        const res = await fetch(
          ...
          { headers: { 'User-Agent': 'Scorvex/1.0' } }
        );
        if (!res.ok) return;
        const data = await res.json();
        for (const ev of (data.events || [])) {
          if (!seen.has(ev.id)) {
            seen.add(ev.id);
            allMatches.push(parseEvent(ev, league));
          }
        }
...
async function handleLeagueRounds(url, env) {
  ...
  try {
    ...
    const res  = await fetch(`${ESPN_BASE}/${league}/scoreboard${seasonQuery}`, { headers: { 'User-Agent': 'Scorvex/1.0' } });
...
async function runHistoricalSync(env, maxOps = 6) {
  ...
  if (state.phase === 'standings') {
    ...
    while (opsLeft > 0) {
      ...
      const kvKey  = `standings_v10_${league}_${season}`;
...
async function handleSitemap(url, env) {
  const origin = 'https://scorvex.netlify.app';
  const urls = new Set([
    `${origin}/`,
    `${origin}/index.html`,
    `${origin}/league.html`,
  ...
  // Stable league navigation pages.
  for (const league of Object.keys(LEAGUE_NAMES)) {
    const q = `league=${encodeURIComponent(league)}&lang=ar`;
    urls.add(`${origin}/league.html?${q}&view=standings`);
    urls.add(`${origin}/league.html?${q}&view=scorers`);
    urls.add(`${origin}/league.html?${q}&view=matches`);
  }

  // Build a useful match sitemap from the same daily KV caches used by
  // /api/matches. We cover a wider window so the sitemap is not limited to
  ...
  await Promise.all(dayKeys.map(async (dateParam) => {
    const key = `matches_v6_${dateParam.replace(/-/g, '')}`;

    try {
      let data = await kvGet(env, key);

      // If the day's data is not already cached, fetch it once from ESPN.
      if (!data) {
        const res = await fetch(
          `${ESPN_ALL}?dates=${dateParam.replace(/-/g, '')}&limit=500`,
          { headers: { 'User-Agent': 'Scorvex/1.0' } }
        );
        if (!res.ok) return;
        ...
        data = { success: true, date: dateParam, count: matches.length, matches };

        // Keep the normal cache policy used by the existing matches endpoint.
        const hasLive = matches.some(m => m?.status === 'in');
        const today = dateParam === todayEspn();
        await kvPut(
          env,
          key,
          data,
          hasLive ? TTL_LIVE : today ? TTL_MATCHES : TTL_FINISHED
        );
      }

      for (const m of (data?.matches || [])) {
        if (!m?.id || !m?.league || !m?.date) continue;
        ...
        urls.add(`${origin}/match.html?${qs.toString()}`);
      }
    } catch {
      // One unavailable day must not break the whole sitemap.
    }
  }));
  ...
  const xml = [
    ...
    ...Array.from(urls).sort().map(u =>
      `  <url><loc>${xmlEscape(u)}</loc></url>`
    ),
    '</urlset>'
  ].join('\n');

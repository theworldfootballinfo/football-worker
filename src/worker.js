// ═══════════════════════════════════════════════════════════════════════════════
// worker.js — Scorio Cloudflare Worker (النسخة التي تعمل)
// ═══════════════════════════════════════════════════════════════════════════════
...
function parseEvent(ev) {
  ...
  // استخراج اسم الدوري من altGameNote أو من leagues
  let leagueName = '';
  let leagueCode = '';
  
  // 1. محاولة من altGameNote (مثل "Carabao Cup, First Round")
  if (comp.altGameNote) {
    const parts = comp.altGameNote.split(',').map(s => s.trim());
    leagueName = parts[0] || '';
  }
  
  // 2. محاولة من leagues array
  ...
  // 4. إذا لم نجد اسم الدوري، استخدم اسم المباراة
  if (!leagueName) {
    leagueName = ev.name || '';
  }
...
async function handleMatches(url, env) {
  ...
  try {
    ...
    // حفظ في KV
    const ttl = hasLive ? 120 : isToday ? 300 : null;
...
async function handleStandings(url, env) {
  ...
  try {
    const res = await fetch(`https://site.web.api.espn.com/apis/v2/sports/soccer/${league}/standings`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; Scorio/1.0)',
      }
    });
...
async function handleScorers(url, env) {
  ...
  const kvKey = `scorers_v5_${league}_${season}`;
  ...
  try {
    const res = await fetch(`https://site.web.api.espn.com/apis/v2/sports/soccer/${league}/statistics?season=${season}`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; Scorio/1.0)',
      }
    });

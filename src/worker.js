...
const ID_TO_CODE = {
  ...
  '630':'bra.1','640':'chi.1','650':'col.1','660':'ecu.1','670':'per.1',
  '680':'uru.1','620':'bol.1','745':'arg.1','760':'mex.1',
  '3913':'den.1','3930':'irl.1','3932':'mex.2','3934':'par.1',
  '3939':'rus.1','3945':'swe.1','4002':'usa.2','4003':'arg.3',
  '5672':'afc.asean','8301':'usa.nwsl',
  '11088':'bra.3','23286':'can.1','221':'can.1','19834':'friendly',
  '1125':'qat.1','194':'bhr.1','332':'bhr.1',
  '198':'omn.1','335':'omn.1','206':'jor.1','333':'jor.1',
  '210':'irq.1','343':'irq.1','214':'lbn.1','336':'lbn.1',
  '218':'kwt.1','334':'kwt.1','202':'syr.1','338':'syr.1',
  '341':'irn.1','1123':'egy.1','1121':'mar.1',
  '1118':'alg.1','1133':'tun.1',
  '238':'gha.1','108':'rou.1',
  '113':'irl.1','129':'svk.1','128':'svn.1','166':'ind.1',
  '179':'mas.1','182':'vie.1',
};


// ── جلب slug الدوري من ESPN وتخزينه في KV للأبد ──────────────────
async function resolveLeagueSlug(leagueId, env) {
  ...
  // 3. اسأل ESPN عبر all/scoreboard — يحتوي slug لكل دوري
  try {
    ...
    if (res.ok) {
      ...
      for (const ev of (data.events || [])) {
        // FIX: تحقق من slug مباشرة في leagues[0].slug
...
function resolveLeagueFromName(name) {
  ...
  if (n.includes('argentine lpf')) return 'arg.1';
  ...
  if (n.includes('swedish allsvenskan') || n.includes('allsvenskan')) return 'swe.1';
  if (n.includes('norwegian eliteserien') || n.includes('eliteserien')) return 'nor.1';
  ...
  if (n.includes('j1 league') || n.includes('j league') || n.includes('j-league')) return 'jpn.1';
  ...
  if (n.includes('club friendly')) return 'friendly';
  return '';
}

// ── [FIX ②] الموسم: 2026 افتراضيًا ───────────────────────────
const SEASON_OVERRIDE = {
  ...
  'uefa.euro':'2024','conmebol.copa':'2024','uefa.nations':'2025',
};
...
function getSeasonParam(league) {
  ...
  // FIX: كان 2025، الموسم الحالي 2026
  return '?season=2026';
}
...
const LEAGUE_FLAGS = {
  ...
  'afc.asian.cup':'🏆','afc.champions.elite':'🏆',
};
...
// ── [FIX ①] parseEvent — استخدم slug مباشرة من ESPN ───────────
function parseEvent(ev) {
  ...
  // FIX: ESPN يرسل leagues[0].slug جاهزًا — أفضل بكثير من تحويل الـ ID
  const espnSlug      = ev.leagues?.[0]?.slug || '';
  const uid           = ev.uid || '';
  const leagueId      = uid.match(/~l:(\d+)~/)?.[1] || '';
  const leagueName_raw = ev.leagues?.[0]?.name || (comp.altGameNote || '').split(',')?.[0]?.trim() || '';

  // أولوية: slug مباشر ← قاموس ID ← اسم الدوري ← fallback
  let leagueCode = '';
  if (espnSlug && espnSlug.includes('.')) {
    leagueCode = espnSlug;                          // ✅ slug جاهز من ESPN
  } else if (leagueId && ID_TO_CODE[leagueId]) {
    leagueCode = ID_TO_CODE[leagueId];              // ✅ من القاموس
  } else if (leagueName_raw) {
    leagueCode = resolveLeagueFromName(leagueName_raw) || espnSlug || leagueId || '';
  } else {
    leagueCode = espnSlug || leagueId || '';
  }
...
// ── [FIX ③] /api/summary — أضف resolveLeagueSlug ──────────────
async function handleSummary(url, env) {
  ...
  // FIX: استخدم resolveLeagueSlug مثل بقية الـ handlers
  if (league) {
    league = await resolveLeagueSlug(normalizeLeague(league, leagueName_s), env);
  }
  ...
  // ابدأ بالدوري المعروف إذا وُجد، ثم جرب الأكثر شيوعًا
  ...
  const fallbacks = [
    ...
    'tur.1','sco.1','gre.1','bel.1','mex.1','jpn.1','kor.1',
  ...
  for (const lg of leaguesToTry) {
    try {
      ...
      if (d.header?.competitions?.[0]?.competitors?.length) {
        data = d;
        // FIX: استخرج الـ slug من الاستجابة مباشرة
        ...
        if (espnSlug && espnSlug.includes('.')) {
          usedLeague = espnSlug;                          // ✅ slug جاهز
        } else if (uidId && ID_TO_CODE[uidId]) {
          usedLeague = ID_TO_CODE[uidId];                 // ✅ من القاموس
        } else {
...
async function handleStandings(url, env) {
  ...
  league = await resolveLeagueSlug(normalizeLeague(league, leagueName_h), env);
...
// ── [FIX ④] /api/scorers — أزل الكود الميت if(false) ──────────
async function handleScorers(url, env) {
  let league = url.searchParams.get('league') || 'eng.1';
  const leagueName = url.searchParams.get('leagueName') || '';
  const season = url.searchParams.get('season') || '2026'; // FIX: 2026 افتراضيًا

  // FIX: حذف if(false){...} else if — resolveLeagueSlug يكفي
  league = await resolveLeagueSlug(normalizeLeague(league, leagueName), env);
  ...
  try {
    ...
    // FIX: أضف الموسم السابق كـ fallback
    const tryUrls = [
...
// ── /api/league-matches — FIX: دعم dateTo لجلب نطاق تواريخ ────
async function handleLeagueMatches(url, env) {
  let league  = url.searchParams.get('league') || 'eng.1';
  const leagueName_m = url.searchParams.get('leagueName') || '';
  const date    = url.searchParams.get('date')   || todayEspn();
  const dateTo  = url.searchParams.get('dateTo') || date; // FIX: دعم نطاق التواريخ
  league = await resolveLeagueSlug(normalizeLeague(league, leagueName_m), env);

  // إذا كان نطاق تواريخ، استخدم مفتاح cache مختلف
  const cacheKey  = dateTo !== date ? `lgm_v2_${league}_${date}_${dateTo}` : `lgm_v2_${league}_${date}`;
  const isToday = date === todayEspn();
  ...
  try {
    ...
    if (dateTo === date) {
      ...
      // نطاق تواريخ — ESPN يقبل dates=YYYYMMDD-YYYYMMDD
      ...
      // إذا لم يرجع نتائج، جرب كل يوم بشكل منفصل

...
const ESPN_SUMMARY_ALL = `${ESPN_BASE}/all/summary`;
...
function getStandingRank(entry, stats, fallback) {
  const candidates = [
    stats.rank,
    stats.position,
    entry?.rank,
...
async function handleMatches(url, env) {
  ...
  try {
    ...
    for (const endpoint of [
      `${ESPN_ALL}?dates=${date}&limit=500`,
      `${ESPN_API_BASE}/all/scoreboard?dates=${date}&limit=500`,
    ]) {
      try {
        const res = await fetch(endpoint, { headers: { 'User-Agent': 'Scorio/1.0' } });
        if (!res.ok) continue;
    ...
    if (!data) throw new Error('ESPN scoreboard unavailable');
...
async function handleSummary(url, env) {
  ...
  // ESPN exposes a competition-independent summary endpoint. Try it first
  // so friendlies and less common cups do not depend on a guessed league code.
  for (const endpoint of [
    `${ESPN_SUMMARY_ALL}?event=${encodeURIComponent(matchId)}`,
    `${ESPN_API_BASE}/all/summary?event=${encodeURIComponent(matchId)}`,
  ]) {
    if (data) break;
    try {
      const res = await fetch(endpoint, { headers: { 'User-Agent': 'Scorio/1.0' } });
      if (!res.ok) continue;
      const candidate = await res.json();
      if (candidate.header?.competitions?.[0]?.competitors?.length) {
        data = candidate;
        usedLeague = resolveLeagueFromName(candidate.header?.league?.name || '') || league;
      }
    } catch {}
  }
...
async function handleStandings(url, env) {
  ...
  const kvKey = requestedSeason
    ? `standings_v8_${league}_${requestedSeason}`
    : `standings_v8_${league}_current`;
  const cached = await kvGet(env, kvKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });
  ...
  for (const child of (data.children || [])) {
    ...
    const teams = entries.map((e, i) => {
      ...
      const rank = getStandingRank(e, stats, i + 1);
  ...
  const result = {
    ...
    groups,
    // Keep an explicit rank → color map for clients that render the color
    // from the table position instead of the team object.
    color_map: groups.reduce((map, group) => {
      group.teams.forEach(team => {
        if (team.note_color) map[team.rank] = team.note_color;
      });
      return map;
    }, {}),
    legend,

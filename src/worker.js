...
// ESPN currently returns 403 for site.api.espn.com from some hosted IPs.
// The web API host exposes the same JSON endpoints and works from Workers.
// Do not add a browser User-Agent: the Worker should use its native fetch
// identity rather than impersonating Chrome.
const ESPN_HOST = 'https://site.web.api.espn.com';
const ESPN_ALL = `${ESPN_HOST}/apis/site/v2/sports/soccer/all/scoreboard`;
const ESPN_BASE = `${ESPN_HOST}/apis/site/v2/sports/soccer`;
const ESPN_STAND = `${ESPN_HOST}/apis/v2/sports/soccer`;
...
function normalizeEspnDate(value, fallback = todayEspn()) {
...
function matchCacheKey(date, lang = 'ar') {
  const safeLang = SUPPORTED_LANGS.includes(lang) ? lang : 'ar';
  return `matches_${MATCH_CACHE_VERSION}_${normalizeEspnDate(date)}_${safeLang}`;
}

function roundsCacheKey(league, season) {
  return `rounds_${MATCH_CACHE_VERSION}_${league}_${season}`;
}
...
async function fetchJson(url, options = {}, attempts = 3) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      ...
      // Retry rate limits and temporary upstream failures, not bad requests.
  ...
  return { res: null, data: null, status: lastStatus, error: lastError };
}
...
async function resolveLeagueSlug(leagueId, env, nameHint = '') {
  ...
  try {
    ...
    const result = await fetchJson(`${ESPN_ALL}?dates=${today}&limit=500`);
    if (result.data) {
...
function resolveLeagueFromName(name) {
  ...
  if (n.includes('fa cup') || n.includes('emirates fa cup')) return 'eng.fa';
  if (n.includes('community shield') || n.includes('charity shield')) return 'eng.community_shield';
  if (n.includes('copa del rey')) return 'esp.copa_del_rey';
  if (n.includes('coppa italia')) return 'ita.coppa_italia';
  if (n.includes('dfb-pokal') || n.includes('german cup')) return 'ger.dfb_pokal';
  if (n.includes('coupe de france') || n.includes('french cup')) return 'fra.coupe_de_france';
  ...
  if (n.includes('women') && n.includes('champions league')) return 'uefa.wchampions';
...
async function handleMatches(url, env) {
  const date = normalizeEspnDate(url.searchParams.get('date'));
  const lang = url.searchParams.get('lang') || 'ar';
  const kvKey = matchCacheKey(date, lang);
  const isToday = date === todayEspn();
  const cached = await kvGet(env, kvKey);
  // A cached empty result is not authoritative for today. Retry ESPN so a
  // transient outage during a scheduled refresh cannot make the UI stay empty.
  if (cached && (cached.count > 0 || !isToday)) {
    return jsonResp({ ...cached, fromCache: true });
  }
  try {
    const result = await fetchJson(`${ESPN_ALL}?dates=${date}&limit=500`);
    if (!result.data) {
      const status = result.res?.status || result.status || 502;
      return jsonResp({
        success: false, date, count: 0, matches: [],
    ...
    const payload = {
      success: true, date, count: matches.length, matches: matchesWithKeywords
    };
    await kvPut(env, kvKey, payload,
      hasLive ? TTL_LIVE() : isToday ? TTL_MATCHES : TTL_FINISHED);
    return jsonResp(payload);
  } catch (e) {
    return jsonResp({ success: false, date, count: 0, matches: [], error: e.message }, 502);
  }
}
...
async function handleSummary(url, env) {
  ...
  // The aggregate summary endpoint resolves cup and smaller competitions
  // without requiring a guessed league slug.
  const endpointsToTry = [
    `${ESPN_BASE}/all/summary?event=${encodeURIComponent(matchId)}`,
    ...[...primary, ...fallbacks.filter(l => l !== league)]
      .map(lg => `${ESPN_BASE}/${lg}/summary?event=${encodeURIComponent(matchId)}`)
  ];
  ...
  for (const endpoint of endpointsToTry) {
    try {
      ...
      if (d.header?.competitions?.[0]?.competitors?.length) {
        ...
        else usedLeague = resolveLeagueFromName(d.header?.league?.name || '') || league || 'eng.1';
...
async function handleScorers(url, env) {
  ...
  const urls = [
    `${ESPN_STAND}/${league}/leaders?season=${season}`,
    `${ESPN_STAND}/${league}/statistics?season=${season}`,
    `${ESPN_HOST}/apis/site/v2/sports/soccer/${league}/leaders?season=${season}`,
  ];
  for (const endpoint of urls) {
    try {
      const result = await fetchJson(endpoint);
      if (!result.data) continue;
      const scorers = extractScorers(result.data);
      if (!scorers.length) continue;
      const payload = { success: true, league, season, scorers };
      await kvPut(env, kvKey, payload, TTL_SCORERS);
      await kvPut(env, backupKey, payload, TTL_SCORERS_BACKUP);
      return jsonResp(payload);
    } catch {}
  }
  return jsonResp({ success: false, league, season, scorers: [],
    error: 'تعذر جلب الهدافين من ESPN' }, 502);
}
...
async function handleRoundsSet(url, env, request) {
  ...
  try {
    const body = await request.json();
    const kvKey = roundsCacheKey(league, season);
...
async function fetchStandingsForSeason(league, season) {
  const urls = [
    ...
    `${ESPN_STAND}/${league}/standings?season=${parseInt(season) - 1}`,
  ];
  for (const u of urls) {
    try {
      const res = await fetch(u);
...
async function runHistoricalSync(env, maxOps = 6) {
  ...
  if (state.phase === 'matches') {
    ...
    while (opsLeft > 0 && dateToEspn(cursor) <= todayStr) {
      ...
      const checkKey = matchCacheKey(dayStr);
      const already = await kvGet(env, checkKey);
      if (!already) {
        try {
          const result = await fetchJson(`${ESPN_ALL}?dates=${dayStr}&limit=500`);
          if (result.data) {
            const matches = (result.data.events || []).map(parseEvent);
            await kvPut(env, checkKey, { success: true, date: dayStr, count: matches.length, matches }, TTL_FINISHED);
          }
        } catch {}
        opsLeft--;
        if (opsLeft > 0) await new Promise(r => setTimeout(r, 1000));
      }
  ...
  if (state.phase === 'standings') {
    ...
    while (opsLeft > 0) {
      ...
      if (!already) {
        ...
        opsLeft--;
        if (opsLeft > 0) await new Promise(r => setTimeout(r, 750));
      }
      leagueIdx++;
    }
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    if (cron === '*/2 * * * *') {
      ...
      if (!histState || histState.phase !== 'done') { ctx.waitUntil(runHistoricalSync(env, 4)); }
    } else {
...
async function handleLeagueMatches(url, env) {
  ...
  const lang = url.searchParams.get('lang') || 'ar';
  const date = normalizeEspnDate(url.searchParams.get('date'));
  const dateTo = normalizeEspnDate(url.searchParams.get('dateTo'), date);
  league = await resolveLeagueSlug(normalizeLeague(league, leagueName_m), env, leagueName_m);
  const cacheKey = `lgm_v4_${league}_${date}_${dateTo}_${lang}`;
  const cached = await kvGet(env, cacheKey);
  if (cached) return jsonResp({ ...cached, fromCache: true });

  if (dateTo < date) {
    return jsonResp({ success: false, error: 'dateTo must be on or after date' }, 400);
  }

  try {
    const result = await fetchJson(
      `${ESPN_ALL}?dates=${date}-${dateTo}&limit=500`
    );
    if (!result.data) {
      const status = result.res?.status || result.status || 502;
      return jsonResp({ success: false, league, date, dateTo, matches: [],
        count: 0, error: `ESPN returned HTTP ${status}` }, status);
    }

    const matches = (result.data.events || [])
      .map(parseEvent)
      .filter(match => match.league === league)
      .map(match => ({
        ...match,
        keywords: generateMatchKeywords(match, lang),
        leagueNameTranslated: LEAGUE_NAMES_I18N[match.league]?.[lang] || match.leagueName
      }));
    const payload = { success: true, league, date, dateTo, count: matches.length, matches };
    await kvPut(env, cacheKey, payload, dateTo === date ? TTL_MATCHES : TTL_ROUND_MATCHES);
    return jsonResp(payload);
  } catch (e) {
    return jsonResp({ success: false, league, date, dateTo, matches: [],
      count: 0, error: e.message }, 502);
  }
}
...
async function handleLeagueRounds(url, env) {
  ...
  try {
    const result = await fetchJson(`${ESPN_BASE}/${league}/scoreboard`);
    if (!result.data) {
      const status = result.res?.status || result.status || 502;
      return jsonResp({ success: false, league, season, rounds: [],
        error: `ESPN returned HTTP ${status}` }, status);
    }
    const calendar = result.data.leagues?.[0]?.calendar || [];
    const rounds = [];
    if (Array.isArray(calendar) && calendar.length) {
      if (calendar[0]?.entries) {
        calendar[0].entries.forEach((entry, index) => {
          const from = (entry.startDate || '').slice(0, 10);
          const to = (entry.endDate || entry.startDate || '').slice(0, 10);
          if (from) rounds.push({
            number: index + 1,
            label: entry.label || `دور ${index + 1}`,
            detail: entry.detail || '',
            dateFrom: from,
            dateTo: to || from
          });
        ...
        const flush = () => {
          ...
          rounds.push({
            ...
            label: `الجولة ${rounds.length + 1}`,
            dateFrom: group[0],
            dateTo: group[group.length - 1]
          });
          group = [];
        };
        dates.forEach((item, index) => {
          const previous = dates[index - 1];
          if (previous && (new Date(item) - new Date(previous)) / 86400000 >= 4) flush();
          group.push(item);
        });
        flush();
      }
    }
    const payload = { success: true, league, season, type: 'rounds', rounds };
    await kvPut(env, kvKey, payload, TTL_ROUNDS);
    return jsonResp(payload);
  } catch (e) {
    return jsonResp({ success: false, league, season, rounds: [], error: e.message }, 502);
  }
}
...
async function handleHistoricalSync(url, env, ctx) {
  ...
  const maxOps = Math.min(parseInt(url.searchParams.get('maxOps') || '4', 10), 8);
...
async function runSeasonSync(env) {
  ...
  while (cursor <= endDate) {
    ...
    try {
      const result = await fetchJson(`${ESPN_ALL}?dates=${fromStr}-${toStr}&limit=500`);
      if (result.data) {
        const data = result.data;
        ...
        for (const [dayKey, matches] of Object.entries(byDate)) {
          const kvKey = matchCacheKey(dayKey);

...
// استخدم site.web.api: site.api يرجع HTML/403 عند استدعائه من Cloudflare Workers
...
// ── /api/scorers ───────────────────────────────────────────────
// ✅ مُصلَّح: يستخدم /statistics بدل /leaders مع fallback
async function handleScorers(url, env) {
  ...
  try {
    const sp = season
      ? `?season=${season}`
      : getSeasonParam(league);

    // نجرب /statistics أولاً ثم /leaders كـ fallback
    const urlsToTry = [
    ...
    for (const u of urlsToTry) {
      try {
        ...
        // /statistics endpoint → data.stats[].leaders[]
        ...
        // /leaders endpoint → data.categories[].leaders[]
    ...
    if (!scorers.length) {
      // لا توجد بيانات هدافين لهذا الدوري
      return jsonResp({
        success: false,
        noData: true,
        scorers: [],
...
// ── /api/league-matches ────────────────────────────────────────
// ✅ مُصلَّح: يجلب كل أيام الجولة (dateFrom → dateTo)
async function handleLeagueMatches(url, env) {
  ...
  // بناء قائمة بكل التواريخ في النطاق (بحد أقصى 7 أيام)
...
async function handleLeagueRounds(url, env) {
  ...
  try {
    ...
    // Entries-based calendar (cups/UCL/leagues with matchweeks)
    ...
    // Date-based calendar (leagues with plain date strings)
    const rawDates = [];
...
export default {
  async fetch(request, env) {
    ...
    // مسارات توافقية للرابط القديم المستخدم في بعض نسخ الواجهة
    if (
      path === '/api/v2/matches/all' ||
      path === '/api/v2/matches/all/' ||
      path === '/api/v2/matches/all//'
